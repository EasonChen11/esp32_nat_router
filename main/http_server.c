...
httpd_handle_t start_webserver(void)
{
    ...
    // create dynamic ssid list for the page
    int ssid_count = global_ssid_list_len;
    char *ssid_options = malloc(2048); // 分配一个大约的初始大小
    strcpy(ssid_options, "");          // 初始化字符串
    // 生成下拉选单的每个选项
    for (int i = 0; i < ssid_count; i++)
    {
        char option[128]; // 分配足够大的空间来存储一个<option>标签
        snprintf(option, sizeof(option), "<option value='%s'>%s</option>", global_ssid_list[i], global_ssid_list[i]);
        strcat(ssid_options, option);
    }
    ...
    snprintf(
        ...
        static_ip, subnet_mask, gateway_addr, ssid_options);
